      ...
      { // first packet
        ...
        else if (chan >= 0x40) // dynamic channel
        {
          ...
          if ((sflag & IN_SSAREP) != 0 && (buf[9] == 1 || buf[9] == 3 || buf[9] == 5 || buf[9] == 7) && buf[10] == 0)
            ...
            gotflag = IN_PNRSP;
          else if (buf[10] == 0xEF && buf[9] == 0x03 && (buf[12] == 0xE1 || buf[12] == 0xE3 || buf[12] == 0x93 || buf[12] == 0x91))
          { // copy for reply
    ...
    do
    {
      ...
      if (cticn >= 0)
      {
        ...
        else
        {
          ...
          else if (handle == cp->chandle && cp->uuidtype == 2 && dat[5] == cp->uuid[1] && dat[6] == cp->uuid[0] && datcount == cp->size && bincmp(cp->value, dat + 7, datcount, DIRN_FOR) != 0)
          {
